...
def _render_generation_controls(
    ...
    if start_button:
        ...
        if params.video_source not in ["pexels", "pixabay", "coverr", "local", "seedance"]:
            _remove_active_generation_task(task_id)
...
def _render_quick_mode():
    """6-step quick mode: input -> 5 ideas -> style -> reference -> prompt -> generate"""
    import os as _os, tempfile as _tempfile

    # Init session state
    defaults = {
        "quick_mode_step": "input", "quick_mode_ideas": [], "quick_mode_selected": -1,
        "quick_mode_style": "写实商业广告", "quick_mode_refs": [],
        "quick_mode_prompt": "", "quick_gen_task_id": None,
    }
    for k, v in defaults.items():
        if k not in st.session_state:
            st.session_state[k] = v

    with st.expander(":zap: 快速模式 - 一键生成视频", expanded=True, icon=":material/bolt:"):
        st.caption("输入活动描述，AI 自动生成创意、风格、分镜提示词")
        ...
        if st.session_state.quick_mode_step == "input":
            ...
            with col1:
                quick_input = st.text_input(
                    "活动描述",
                    placeholder="例如：充100送20，进店领鸡蛋，5G套餐半价",
                    key="quick_mode_input",
                    label_visibility="collapsed",
                )
            with col2:
                if st.button(":sparkles: 生成创意方案", use_container_width=True, type="primary", disabled=not quick_input):
                    with st.spinner("正在生成 5 个创意方案..."):
                        try:
        ...
        # ============================================================
        # ============================================================
        # Step 2: Choose Creative (6 cards, 3+3 layout)
        # ============================================================
        elif st.session_state.quick_mode_step == "choose":
            ideas = st.session_state.quick_mode_ideas
            st.success("已生成 {} 个创意方案，请选择一个".format(len(ideas)))
            ...
            # Row 1: 3 cards
            cols = st.columns(3)
            for idx in range(min(3, len(ideas))):
                idea = ideas[idx]
                label = style_labels.get(idea.get("style", ""), "[?]")
                with cols[idx]:
                    with st.container(border=True):
                        st.markdown("{} **{}**".format(label, idea.get("title", "")))
                        st.caption(idea.get("description", "")[:150] + "...")
                        if st.button("选择此方案", key="sel_{}".format(idx), use_container_width=True):
        ...
        # ============================================================
        # Step 3: Style Selection (6 styles, 3+3 layout)
        # ============================================================
            ...
            if st.session_state.quick_mode_selected >= 0:
                ...
                st.info("已选方案: **{}**".format(idea.get("title", "")))

            st.markdown("### 选择视频风格")

            styles = [
                ("写实商业广告", "明亮专业，适合促销活动和产品展示"),
                ("电影级写实", "柔和光影浅景深，适合品牌宣传大片"),
                ("手机实拍风", "手持感自然光，适合探店日常记录"),
                ("2D可爱动画", "圆润线条明亮配色，适合趣味活动"),
                ("复古胶片风", "颗粒质感暖黄调，适合怀旧主题"),
                ("3D科技质感", "光滑表面科技感，适合产品功能演示"),
            ]

            # Row 1
            cols = st.columns(3)
            for idx in range(3):
                s_name, s_desc = styles[idx]
                with cols[idx]:
                    selected = st.session_state.quick_mode_style == s_name
                    if st.button("{}\n{}".format(s_name, s_desc), key="style_{}".format(idx),
                                 use_container_width=True,
                                 type="primary" if selected else "secondary"):
                        st.session_state.quick_mode_style = s_name
                        st.rerun()
            ...
            st.markdown("已选择: **{}**".format(st.session_state.quick_mode_style))

            c1, c2 = st.columns(2)
            with c1:
                if st.button("返回重选方案", use_container_width=True):
                    st.session_state.quick_mode_step = "choose"
                    st.rerun()
            with c2:
                if st.button("下一步：添加参考图", use_container_width=True, type="primary"):
                    st.session_state.quick_mode_step = "reference"
                    st.rerun()
        # Step 4: Reference Image Upload (optional, one at a time via popover)
        # ============================================================
            ...
            st.markdown("### :frame_with_picture: 参考图（可选）")
            st.caption("上传门店照片、产品图等作为视频参考，AI 会根据图片名称生成对应的提示词。不上传则直接跳过。")
            ...
            if refs:
                ...
                for ri, ref in enumerate(refs):
                    ...
                    with c1:
                        st.markdown(":white_check_mark: **{}**".format(ref.get("name", "未命名")))
                    with c2:
                        if ref.get("path"):
                            st.caption("图片已上传: {}".format(ref["path"].split(chr(92))[-1][:40]))
                    with c3:
                        if st.button(":x:", key="del_ref_{}".format(ri), help="删除此参考图"):
                            st.session_state.quick_mode_refs.pop(ri)
                            st.rerun()

            # Add reference via popover
            with st.popover(":heavy_plus_sign: 添加参考图", use_container_width=False):
                st.markdown("**添加一张参考图**")
                ref_name = st.text_input("图片名称（AI 用于生成提示词）",
                                         placeholder="例如：电信营业厅门店照片、银渐层猫咪",
                                         key="new_ref_name")
                ref_file = st.file_uploader("选择图片", type=["jpg", "jpeg", "png", "webp"],
                                            key="new_ref_file", label_visibility="collapsed")
                if ref_file and ref_name:
                    if st.button(":floppy_disk: 保存此参考图", use_container_width=True, type="primary",
                                 key="save_ref_btn"):
                        ...
                        st.session_state.quick_mode_refs.append({
                            "name": ref_name,
                            "path": save_path,
                        })
                        st.rerun()
            ...
            with c1:
                if st.button(":arrow_left: 返回选风格", use_container_width=True):
                    st.session_state.quick_mode_step = "style"
                    st.rerun()
            with c2:
        ...
        # ============================================================
            ...
            if not st.session_state.quick_mode_prompt:
                ...
                edited = st.text_area(
                    "提示词",
                    value=st.session_state.quick_mode_prompt,
                    height=350,
                    key="quick_mode_edited_prompt",
                    label_visibility="collapsed",
                )
                ...
                with c1:
                    if st.button(":arrow_left: 返回", use_container_width=True):
                        ...
                        st.rerun()
                with c2:
                    if st.button(":repeat: 重新生成", use_container_width=True):
                        st.session_state.quick_mode_prompt = ""
                        st.rerun()
                with c3:
                    if st.button(":rocket: 确认生成视频", use_container_width=True, type="primary"):
        ...
        # ============================================================
            ...
            st.info(":hourglass_flowing_sand: 正在生成视频...")
            with st.expander("查看提示词", expanded=False):
                st.code(final_prompt[:300] + "...", language=None)

            if "quick_gen_task_id" not in st.session_state or not st.session_state.quick_gen_task_id:
                ...
                try:
                    ...
                    st.session_state.quick_gen_task_id = r.json()["data"]["task_id"]
                    st.rerun()
                except Exception as e:
                ...
                task_id = st.session_state.quick_gen_task_id
                import requests as _req, time as _time
                try:
                    r = _req.get("http://127.0.0.1:8080/api/v1/tasks/" + task_id, timeout=10)
                    task = r.json().get("data", {})
                    state = task.get("state", -1)
                    progress = task.get("progress", 0)
                    st.progress(progress / 100.0, "进度: {}%".format(progress))

                    if state == 1:
                        st.success(":tada: 视频生成完成!")
                        videos = task.get("videos", [])
                        for idx, v in enumerate(videos, 1):
                            url = "http://127.0.0.1:8080/tasks/{}/final-{}.mp4".format(task_id, idx)
                            st.video(url)
                        if st.button(":repeat: 重新生成", key="gen_restart"):
                            st.session_state.quick_mode_step = "input"
                            st.session_state.quick_mode_ideas = []
                            st.session_state.quick_mode_prompt = ""
                            st.session_state.quick_gen_task_id = None
