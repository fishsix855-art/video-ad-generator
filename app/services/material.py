...
# =============================================================================
# Seedance Material Provider (Phase 3)
# =============================================================================

class SeedanceMaterialProvider:
    ...
    def __init__(self, api_key=None, model=None):
        try:
            ...
            raise ImportError(
                "volcengine-python-sdk[ark] is required. Install with: "
                "pip install volcengine-python-sdk[ark]"
            )
        self._api_key = api_key or config.app.get("volcengine_api_key", "")
        self._model = model or config.app.get(
            "volcengine_model_name", "doubao-seedance-1-0-pro-250528"
        )
        if not self._api_key:
            raise ValueError(
                "volcengine_api_key is not set. Please set it in config.toml"
            )
        self._client = Ark(
    ...
    def generate_video(
        ...
        while waited < timeout_seconds:
            ...
            logger.info("  Seedance [{}s] status: {}".format(waited, task.status))
    ...
    def download_video(self, video_url, save_dir=""):
        ...
        video_dir = save_dir or utils.storage_dir("seedance_videos", create=True)
        filename = "seedance_{}.mp4".format(hash(video_url) % 1000000)
        video_path = os.path.join(video_dir, filename)

        logger.info("Downloading Seedance video to {}".format(video_path))
        r = _requests.get(video_url, timeout=300, verify=_get_tls_verify())
        r.raise_for_status()
        with open(video_path, "wb") as f:
            f.write(r.content)
        logger.info("Downloaded: {} KB".format(len(r.content) // 1024))
        return video_path
